...
class CleanTests(PorcelainTestCase):
    ...
    def assert_wd(self, expected_paths):
        ...
        found_paths = {
            os.path.normpath(p) for p in flat_walk_dir(self.repo.path) if not p.split(os.sep)[0] == control_dir_rel
        }
...
class StatusTests(PorcelainTestCase):
    ...
    def test_get_untracked_paths(self):
        ...
        self.assertEqual(
            ...
            set(porcelain.get_untracked_paths(self.repo.path, self.repo.path, self.repo.open_index())),
        )
        self.assertEqual(
    ...
    def test_get_untracked_paths_subrepo(self):
        ...
        self.assertEqual(
            ...
            set(porcelain.get_untracked_paths(subrepo.path, subrepo.path, subrepo.open_index())),
        )
        self.assertEqual(
            set(),
            set(
...
class FetchTests(PorcelainTestCase):
    ...
    def assert_correct_remote_refs(self, local_refs, remote_refs, remote_name=b"origin"):
        ...
        locally_known_remote_refs = {
            k[len(remote_ref_prefix) + 1 :]: v for k, v in local_refs.items() if k.startswith(remote_ref_prefix)
        }

        normalized_remote_refs = {
            k[len(local_ref_prefix) + 1 :]: v for k, v in remote_refs.items() if k.startswith(local_ref_prefix)
        }
