...
def pack_refs(repo, all=False):
    with open_repo_closing(repo) as repo_obj:
        refs = repo_obj.refs
        packed_refs = {ref: refs[ref] for ref in refs if (all or ref.startswith(LOCAL_TAG_PREFIX)) and ref != b"HEAD"}
    ...
    **kwargs,
    ...
    with open_repo_closing(repo) as r:
        ...
        def determine_wants(remote_refs, **kwargs):
            ...
            return [remote_refs[lh] for (lh, rh, force_ref) in selected_refs if remote_refs[lh] not in r.object_store]
...
def status(repo=".", ignored=False, untracked_files="all"):
    ...
    with open_repo_closing(repo) as r:
        ...
        if sys.platform == "win32":
            untracked_changes = [path.replace(os.path.sep, "/") for path in untracked_paths]
        else:
...
def checkout_branch(repo, target: Union[bytes, str], force: bool = False):
    ...
    if force:
        ...
        changes = list(
            set(status_report[0]["add"] + status_report[0]["delete"] + status_report[0]["modify"] + status_report[1])
        )
        index = 0
        while index < len(changes):
            ...
            try:
                ...
                try:
                    ...
                    raise CheckoutError(
                        "Your local changes to the following files would be overwritten by checkout: " + change.decode()
                    )
            except KeyError:
                changes.pop(index)
