    ...
    def sign(self, keyid: Optional[str] = None):
        ...
        with gpg.Context(armor=True) as c:
            if keyid is not None:
                ...
                self.signature, unused_result = c.sign(self.as_raw_string(), mode=gpg.constants.sig.mode.DETACH)
    ...
    def check(self):
        ...
        for name, mode, sha in parse_tree(b"".join(self._chunked_text), True):
            ...
            if b"/" in name or name in (b"", b".", b"..", b".git"):
                raise ObjectFormatException("invalid name %s" % name.decode("utf-8", "replace"))
    ...
    def sign(self, keyid: Optional[str] = None):
        ...
        with gpg.Context(armor=True) as c:
            if keyid is not None:
                ...
                self.gpgsig, unused_result = c.sign(self.as_raw_string(), mode=gpg.constants.sig.mode.DETACH)
