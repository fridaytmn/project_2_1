...
class ConfigDict(Config, MutableMapping[Section, MutableMapping[Name, Value]]):
    ...
    def __init__(
        self,
        values: Union[MutableMapping[Section, MutableMapping[Name, Value]], None] = None,
    ...
    def items(self, section: Section) -> Iterator[Tuple[Name, Value]]:  # type: ignore[override]
    ...
    while i < len(value):
        ...
        if c == ord(b"\\"):
            ...
            try:
                ...
                raise ValueError(
                    "escape character followed by unknown character " "%s at %d in %r" % (value[i], i, value)
                ) from exc
            if whitespace:
