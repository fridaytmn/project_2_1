    ...
    * crc32          (if _compute_crc32 is True)
    ...
    def _follow_chain(self, offset: int, obj_type_num: int, base_chunks: List[bytes]):
        ...
        while todo:
            ...
            todo.extend(
                (new_offset, unpacked.obj_type_num, unpacked.obj_chunks) for new_offset in unblocked  # type: ignore
            )
...
def find_reusable_deltas(
    ...
    *,
    ...
    for i, unpacked in enumerate(
        container.iter_unpacked_subset(object_ids, allow_missing=True, convert_ofs_delta=True)
    ):
...
class Pack:
    ...
    def entries(self, progress: Optional[ProgressFn] = None) -> Iterator[PackIndexEntry]:
        ...
        Returns: iterator of tuples with (sha, offset, crc32)
        """
        return self.data.iterentries(progress=progress, resolve_ext_ref=self.resolve_ext_ref)
