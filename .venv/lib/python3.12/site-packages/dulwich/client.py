...
class InvalidWants(Exception):
    ...
    def __init__(self, wants) -> None:
        Exception.__init__(self, "requested wants not in server provided refs: %r" % wants)
        ...
        for refname in new_refs:
            ...
            if old_sha1 != new_sha1:
                ...
                if self.sent_capabilities:
                    ...
                    yield (old_sha1 + b" " + new_sha1 + b" " + refname + b"\0" + b" ".join(sorted(capabilities)))
...
def _handle_upload_pack_head(proto, capabilities, graph_walker, wants, can_read, depth):
    ...
    proto.write_pkt_line(COMMAND_WANT + b" " + wants[0] + b" " + b" ".join(sorted(capabilities)) + b"\n")
    for want in wants[1:]:
        proto.write_pkt_line(COMMAND_WANT + b" " + want + b"\n")
    if depth not in (0, None) or getattr(graph_walker, "shallow", None):
        if CAPABILITY_SHALLOW not in capabilities:
            raise GitProtocolError("server does not support shallow capability required for " "depth")
        ...
        if can_read is not None and can_read():
            ...
            if parts[0] == b"ACK":
                ...
                if parts[2] in (b"continue", b"common"):
                    ...
                    raise AssertionError("%s not in ('continue', 'ready', 'common)" % parts[2])
        have = next(graph_walker)
...
class GitClient:
    ...
    def send_pack(
        ...
        path,
        update_refs,
        generate_pack_data: Callable[[Set[bytes], Set[bytes], bool], Tuple[int, Iterator[UnpackedObject]]],
        progress=None,
    ):
    ...
    def fetch(
        ...
        path: str,
        target: Repo,
        determine_wants: Optional[Callable[[Dict[bytes, bytes], Optional[int]], List[bytes]]] = None,
        progress: Optional[Callable[[bytes], None]] = None,
...
class TraditionalGitClient(GitClient):
    ...
    def send_pack(self, path, update_refs, generate_pack_data, progress=None):
        ...
        with proto:
            ...
            ref_status = self._handle_receive_pack_tail(proto, negotiated_capabilities, progress)
...
class SubprocessSSHVendor(SSHVendor):
    ...
    def run_command(
        ...
        if password is not None:
            raise NotImplementedError("Setting password not supported by SubprocessSSHVendor.")
...
def get_transport_and_path(
    ...
    **kwargs,
    ...
    try:
        return _get_transport_and_path_from_url(location, config=config, operation=operation, **kwargs)
    except ValueError:
        pass
