...
class Container(_CustomDict):
    ...
    def append(self, key: Key | str | None, item: Item, validate: bool = True) -> Container:
        ...
        if isinstance(item, Table):
            ...
            if self._body and not (self._parsed or item.trivia.indent or prev_ws) and not key.is_dotted():
                item.trivia.indent = "\n"
                ...
                if item.is_aot_element():
                    ...
                    if item.is_super_table():
                        ...
                        for k, v in item.value.body:
                            current.append(k, v)
                        self._body[(current_idx[-1] if isinstance(current_idx, tuple) else current_idx)] = (
                            current_body_element[0],
                            current,
                        )
        ...
        if key is not None and self._body and not self._parsed and (not is_table or key.is_dotted()):
    ...
    def _render_table(self, key: Key, table: Table, prefix: str | None = None) -> str:
        ...
        if not table.is_super_table() or (
            any(not isinstance(v, (Table, AoT, Whitespace, Null)) for _, v in table.value.body) and not key.is_dotted()
        ):
            open_, close = "[", "]"
    ...
    def _replace_at(self, idx: int | tuple[int], new_key: Key | str, value: Item) -> None:
        ...
        if isinstance(value, Table):
            ...
            replace_has_ws = isinstance(v, Table) and v.value.body and isinstance(v.value.body[-1][1], Whitespace)
