...
class Parser:
    ...
    def _parse_quoted_key(self) -> Key:
        ...
        key_str = self._parse_string(StringType.SLB if key_type == KeyType.Basic else StringType.SLL)
    ...
    def _parse_number(self, raw: str, trivia: Trivia) -> Item | None:
        ...
        if len(raw) > 1 and (
            raw.startswith("0") and not raw.startswith(("0.", "0o", "0x", "0b", "0e")) or sign and raw.startswith(".")
        ):
            return None
        ...
        if clean.endswith(".") or not clean.startswith("0x") and clean.split("e", 1)[0].endswith("."):
            return None
    ...
    def _parse_string(self, delim: StringType) -> String:
        ...
        while True:
            ...
            if (
                ...
                delim.is_multiline()
                and not escaped
                and (code == CHR_DEL or code <= CTRL_CHAR_LIMIT and code not in [CTRL_I, CTRL_J, CTRL_M])
            ):
    ...
    def _parse_table(self, parent_name: Key | None = None, parent: Table | None = None) -> tuple[Key, Table | AoT]:
        """
        Parses a table element.
        """
        if self._current != "[":
            raise self.parse_error(InternalParserError, "_parse_table() called on non-bracket character.")
