...
def item(value: Any, _parent: Item | None = None, _sort_keys: bool = False) -> Item:
    ...
    if isinstance(value, bool):
        ...
        if value and all(isinstance(v, dict) for v in value) and (_parent is None or isinstance(_parent, Table)):
            a = AoT([])
        ...
        for encoder in CUSTOM_ENCODERS:
            try:
                ...
                if not isinstance(rv, Item):
                    raise ConvertError(f"Custom encoder is expected to return an instance of Item, got {type(rv)}")
                return rv
        ...
        *items: Any,
        ...
        if len(self._value) > 0:
            ...
            last_value_item = next(
                (v for v in self._value[::-1] if v.value is not None and not isinstance(v.value, Null)),
                None,
            )
...
class InlineTable(AbstractTable):
    ...
    def as_string(self) -> str:
        buf = "{"
        last_item_idx = next(
            (i for i in range(len(self._value.body) - 1, -1, -1) if self._value.body[i][0] is not None),
            None,
        )
