...
class HTTPRepository(CachedRepository):
    ...
    def _download(self, url: str, dest: Path, *, raise_accepts_ranges: bool = False) -> None:
        return download_file(url, dest, session=self.session, raise_accepts_ranges=raise_accepts_ranges)

    @contextmanager
    def _cached_or_downloaded_file(self, link: Link, *, raise_accepts_ranges: bool = False) -> Iterator[Path]:
        self._log(f"Downloading: {link.url}", level="debug")
        with temporary_directory() as temp_dir:
            filepath = Path(temp_dir) / link.filename
            self._download(link.url, filepath, raise_accepts_ranges=raise_accepts_ranges)
            yield filepath
    ...
    def _get_info_from_wheel(self, link: Link) -> PackageInfo:
        ...
        if self._lazy_wheel and self._supports_range_requests.get(netloc, True):
            try:
                package_info = PackageInfo.from_metadata(metadata_from_wheel_url(link.filename, link.url, self.session))
        ...
        try:
            with self._cached_or_downloaded_file(link, raise_accepts_ranges=raise_accepts_ranges) as filepath:
        ...
        if link.has_metadata:
            try:
                ...
                if link.metadata_hashes and (
                    ...
                    metadata_hash = getattr(hashlib, hash_name)(response.content).hexdigest()
        ...
        *,
        ...
        if wheels:
            ...
            if universal_wheel is not None:
                return self._get_info_from_metadata(universal_wheel) or self._get_info_from_wheel(universal_wheel)

            info = None
            if universal_python2_wheel and universal_python3_wheel:
                info = self._get_info_from_metadata(universal_python2_wheel) or self._get_info_from_wheel(
                    universal_python2_wheel
                )

                py3_info = self._get_info_from_metadata(universal_python3_wheel) or self._get_info_from_wheel(
            ...
            if universal_python3_wheel:
                return self._get_info_from_metadata(universal_python3_wheel) or self._get_info_from_wheel(
                    universal_python3_wheel
                )

            if universal_python2_wheel:
                return self._get_info_from_metadata(universal_python2_wheel) or self._get_info_from_wheel(
                    universal_python2_wheel
                )

            if platform_specific_wheels:
                first_wheel = platform_specific_wheels[0]
                return self._get_info_from_metadata(first_wheel) or self._get_info_from_wheel(first_wheel)

        return self._get_info_from_metadata(sdists[0]) or self._get_info_from_sdist(sdists[0])

    def _links_to_data(self, links: list[Link], data: PackageInfo) -> dict[str, Any]:
        if not links:
            raise PackageNotFound(
                f'No valid distribution links found for package: "{data.name}" version:' f' "{data.version}"'
            )
        ...
        for link in links:
            ...
            if file_hash is None and (
                hash_type := get_highest_priority_hash_type(set(link.hashes.keys()), link.filename)
            ):
    ...
    def calculate_sha256(self, link: Link) -> str | None:
        with self._cached_or_downloaded_file(link) as filepath:
            hash_name = get_highest_priority_hash_type(set(link.hashes.keys()), link.filename)
            known_hash = None
            ...
            if not hash_name or not known_hash or known_hash.hexdigest() == link.hashes[hash_name]:
