...
class SourceAddCommand(Command):
    ...
    def handle(self) -> int:
        ...
        if is_default and is_secondary:
            self.line_error(
                "<error>Cannot configure a source as both <c1>default</c1> and" " <c1>secondary</c1>.</error>"
            )
            return 1
        ...
        if priority is Priority.SECONDARY:
            allowed_prios = (p for p in Priority if p not in {Priority.DEFAULT, Priority.SECONDARY})
            self.line_error(
