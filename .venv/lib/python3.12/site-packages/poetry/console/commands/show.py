...
lists all packages available."""
    ...
    def handle(self) -> int:
        ...
        if self.option("top-level"):
            if self.option("tree"):
                self.line_error("<error>Error: Cannot use --tree and --top-level at the same" " time.</error>")
                return 1
            if package is not None:
                self.line_error("<error>Error: Cannot use --top-level when displaying a single" " package.</error>")
                return 1

        if self.option("why"):
            if self.option("tree") and package is None:
                self.line_error("<error>Error: --why requires a package when combined with" " --tree.</error>")

                return 1

            if not self.option("tree") and package:
                self.line_error(
                    "<error>Error: --why cannot be used without --tree when displaying" " a single package.</error>"
                )
        ...
        if not self.poetry.locker.is_locked():
            self.line_error("<error>Error: poetry.lock not found. Run `poetry lock` to create" " it.</error>")
            return 1
    ...
    def _display_single_package_information(self, package: str, locked_repository: Repository) -> int:
        ...
        if self.option("tree"):
            if self.option("why"):
                ...
                packages = [pkg]
                if required_by:
                    packages = [p for p in locked_packages for r in required_by if p.name == r]
                else:
                ...
                for p in packages:
                    self.display_package_tree(self.io, p, locked_packages, why_package=pkg)
        ...
        if pkg.requires:
            ...
            for dependency in pkg.requires:
                self.line(f" - <c1>{dependency.pretty_name}</c1>" f" <b>{dependency.pretty_constraint}</b>")
    ...
    def _display_packages_information(self, locked_repository: Repository, root: ProjectPackage) -> int:
        import shutil
        ...
        for locked in locked_packages:
            ...
            if not self.io.output.is_decorated():
                installed_status = self.get_installed_status(locked, installed_repo.packages)
            ...
            if show_latest:
                ...
                update_status = latest_statuses[locked.pretty_name] = self.get_update_status(latest, locked)

                if not self.option("outdated") or update_status != "up-to-date":
                    name_length = max(name_length, current_length)
                    version_length = max(
                        version_length,
                        len(get_package_version_display_string(locked, root=self.poetry.file.path.parent)),
                    )
                    latest_length = max(
                        latest_length,
                        len(get_package_version_display_string(latest, root=self.poetry.file.path.parent)),
                    )
                ...
                if self.option("why"):
                    required_by = reverse_deps(locked, locked_repository)
                    required_by_length = max(required_by_length, len(" from " + ",".join(required_by.keys())))

        write_version = name_length + version_length + 3 <= width
        write_latest = name_length + version_length + latest_length + 3 <= width

        why_end_column = name_length + version_length + latest_length + required_by_length
        ...
        for locked in locked_packages:
            ...
            if show_latest and self.option("outdated") and latest_statuses[locked.pretty_name] == "up-to-date":
                continue

            line = f"<fg={color}>" f"{name:{name_length - len(install_marker)}}{install_marker}</>"
            if write_version:
                version = get_package_version_display_string(locked, root=self.poetry.file.path.parent)
            ...
            if show_latest:
                ...
                if write_latest:
                    ...
                    version = get_package_version_display_string(latest, root=self.poetry.file.path.parent)
            ...
            if write_description:
                ...
                remaining = width - name_length - version_length - required_by_length - 4
    ...
    def _display_packages_tree_information(self, locked_repository: Repository, root: ProjectPackage) -> int:
    ...
    def display_package_tree(
        ...
        for i, dependency in enumerate(dependencies, 1):
            ...
            info = f"{tree_bar}── <{color}>{dependency.name}</{color}>" f" {dependency.pretty_constraint}"
    ...
    def _display_tree(
        ...
        for i, dependency in enumerate(dependencies, 1):
            ...
            info = (
                f"{tree_bar}── <{color}>{dependency.name}</{color}>" f" {dependency.pretty_constraint} {circular_warn}"
            )
            self._write_tree_line(io, info)
    ...
    def find_latest_package(self, package: Package, root: ProjectPackage) -> Package | None:
        from cleo.io.null_io import NullIO
        ...
        return selector.find_best_candidate(name, f">={package.pretty_version}", allow_prereleases)
    ...
    def get_installed_status(self, locked: Package, installed_packages: list[Package]) -> str:
        for package in installed_packages:
