...
<info>poetry remove</info>"""
    ...
    def handle(self) -> int:
        ...
        if self.option("dev"):
            self.line_error(
                "<warning>The --dev option is deprecated, " "use the `--group dev` notation instead.</warning>"
            )
            group = "dev"
        else:
        ...
        if group is None:
            ...
            removed = self._remove_packages(packages, poetry_content["dev-dependencies"], "dev")
        ...
        if not_found:
            raise ValueError("The following packages were not found: " + ", ".join(sorted(not_found)))
    ...
    def _remove_packages(self, packages: list[str], section: dict[str, Any], group_name: str) -> list[str]:
        removed = []
