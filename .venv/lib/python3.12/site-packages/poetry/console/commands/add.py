...
"""
    ...
    def handle(self) -> int:
        ...
        project_name = canonicalize_name(name) if (name := poetry_content.get("name")) else None
        ...
        for _constraint in requirements:
            ...
            if canonical_constraint_name == project_name:
                self.line_error(
                    f"<error>Cannot add dependency on <c1>{constraint_name}</c1> to" " project with the same name."
                )
                self.line_error("\nNo changes were applied.")
                return 1
