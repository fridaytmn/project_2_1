...
class CheckCommand(Command):
    name = "check"
    description = (
        "Validates the content of the <comment>pyproject.toml</> file and its" " consistency with the poetry.lock file."
    )
    ...
    def _validate_classifiers(self, project_classifiers: set[str]) -> tuple[list[str], list[str]]:
        ...
        if deprecated:
            for old_classifier in deprecated:
                ...
                if new_classifiers:
                    message = f"Deprecated classifier {old_classifier!r}. " f"Must be replaced by {new_classifiers!r}."
                else:
