...
To remove a repository (repo is a short alias for repositories):
    ...
    def handle(self) -> int:
        ...
        if m:
            ...
            raise ValueError("You must pass the url. " "Example: poetry config repositories.foo https://bar.com")
        ...
        if m:
            ...
            if m.group(1) == "http-basic":
                if len(values) == 1:
                    ...
                    raise ValueError("Expected one or two arguments " f"(username, password), got {len(values)}")
                else:
                    username = values[0]
                    password = values[1]
                ...
                if len(values) != 1:
                    raise ValueError(f"Expected only one argument (token), got {len(values)}")
        ...
        if m:
            ...
            if self.option("unset"):
                config.auth_config_source.remove_property(f"certificates.{repository}.{key}")
            ...
            if len(values) == 1:
                ...
                config.auth_config_source.add_property(f"certificates.{repository}.{key}", new_value)
            else:
    ...
    def _list_configuration(self, config: dict[str, Any], raw: dict[str, Any], k: str = "") -> None:
        orig_k = k
