...
class PublishCommand(Command):
    ...
    options = [
        option("repository", "r", "The repository to publish the package to.", flag=False),
        ...
        option("cert", None, "Certificate authority to access the repository.", flag=False),
        option(
            "client-cert",
            None,
    ...
    def handle(self) -> int:
        ...
        if self.option("build"):
            if publisher.files and not self.confirm(
                f"There are <info>{len(publisher.files)}</info> files ready for" " publishing. Build anyway?"
            ):
        ...
        if not files:
            self.line_error("<error>No files to publish. " "Run poetry build first or use the --build option.</error>")
        ...
        client_cert = Path(self.option("client-cert")) if self.option("client-cert") else None
