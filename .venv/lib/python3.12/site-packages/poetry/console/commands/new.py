    ...
    def handle(self) -> int:
        ...
        default_python = (
            ...
            + EnvManager.get_python_version(
                precision=2,
                prefer_active_python=poetry_config.get("virtualenvs.prefer-active-python"),
                io=self.io,
            ).to_string()
        )
        ...
        self.line(f"Created package <info>{layout_._package_name}</> in" f" <fg=blue>{path.as_posix()}</>")
