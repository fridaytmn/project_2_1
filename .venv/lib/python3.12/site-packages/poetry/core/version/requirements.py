...
class Requirement:
    ...
    def __init__(self, requirement_string: str) -> None:
        ...
        if url:
            ...
            if parsed_url.scheme == "file":
                if urlparse.urlunparse(parsed_url) != url:
                    raise InvalidRequirement(f'The requirement is invalid: invalid URL "{url}"')
        ...
        try:
            ...
            raise InvalidRequirement(f'The requirement is invalid: invalid version constraint "{constraint}"')

        self.pretty_constraint = constraint

        marker = next(parsed.find_data("marker_spec"), None)
        if marker:
            marker = _compact_markers(marker.children[0].children, tree_prefix="markers__")
