...
SingleMarkerConstraint = TypeVar("SingleMarkerConstraint", bound=Union[BaseConstraint, VersionConstraint])
...
class SingleMarker(SingleMarkerLike[Union[BaseConstraint, VersionConstraint]]):
    ...
    def __init__(self, name: str, constraint: str | BaseConstraint | VersionConstraint) -> None:
        from poetry.core.constraints.generic import (
            parse_constraint as parse_generic_constraint,
        )
        ...
        try:
            ...
            raise InvalidMarker(f"Invalid marker for '{name}': {original_constraint_string}") from e
    ...
    def invert(self) -> BaseMarker:
        if self._operator in ("===", "=="):
            ...
            if not isinstance(self._constraint, VersionRangeConstraint):
                ...
                raise RuntimeError("The '~=' operator should only represent version ranges")
...
class AtomicMultiMarker(SingleMarkerLike[MultiConstraint]):
    ...
    def expand(self) -> MultiMarker:
        return MultiMarker(*(SingleMarker(self._name, c) for c in self._constraint.constraints))

    def __str__(self) -> str:
        return " and ".join(f'{self._name} != "{c.value}"' for c in self._constraint.constraints)


class AtomicMarkerUnion(SingleMarkerLike[UnionConstraint]):
    def __init__(self, name: str, constraint: UnionConstraint) -> None:
        assert all(isinstance(c, Constraint) and c.operator == "==" for c in constraint.constraints)
        super().__init__(name, constraint)
    ...
    def expand(self) -> MarkerUnion:
        return MarkerUnion(*(SingleMarker(self._name, c) for c in self._constraint.constraints))
    ...
    def __str__(self) -> str:
        ...
        return " or ".join(
            f'{self._name} == "{c.value}"' for c in self._constraint.constraints  # type: ignore[attr-defined]
        )
...
class MultiMarker(BaseMarker):
    ...
    def union_simplify(self, other: BaseMarker) -> BaseMarker | None:
        ...
        if isinstance(other, MultiMarker):
            ...
            unique_union = MultiMarker(*unique_markers).union(MultiMarker(*other_unique_markers))
            if isinstance(unique_union, (SingleMarkerLike, AnyMarker)):
                # Use list instead of set for deterministic order.
                common_markers = [marker for marker in self.markers if marker in shared_markers]
...
class MarkerUnion(BaseMarker):
    ...
    def intersect_simplify(self, other: BaseMarker) -> BaseMarker | None:
        ...
        if isinstance(other, MarkerUnion):
            ...
            unique_markers = our_markers - their_markers
            other_unique_markers = their_markers - our_markers
            unique_intersection = MarkerUnion(*unique_markers).intersect(MarkerUnion(*other_unique_markers))
...
def _compact_markers(tree_elements: Tree, tree_prefix: str = "", top_level: bool = True) -> BaseMarker:
    ...
    for token in tree_elements:
        ...
        if token.data == "marker":
            sub_marker = _compact_markers(token.children, tree_prefix=tree_prefix, top_level=False)
...
def cnf(marker: BaseMarker) -> BaseMarker:
    ...
    if isinstance(marker, MarkerUnion):
        ...
        sub_marker_lists = [m.markers if isinstance(m, MultiMarker) else [m] for m in cnf_markers]
        return MultiMarker.of(*[MarkerUnion.of(*c) for c in itertools.product(*sub_marker_lists)])
...
def dnf(marker: BaseMarker) -> BaseMarker:
    ...
    if isinstance(marker, MultiMarker):
        ...
        sub_marker_lists = [m.markers if isinstance(m, MarkerUnion) else [m] for m in dnf_markers]
        return MarkerUnion.of(*[MultiMarker.of(*c) for c in itertools.product(*sub_marker_lists)])
...
def union(*markers: BaseMarker) -> BaseMarker:
    ...
    while isinstance(unnormalized, (MultiMarker, MarkerUnion)) and len(unnormalized.markers) == 1:
...
def _merge_single_markers(
    ...
    if result_constraint.is_empty():
        ...
        isinstance(result_constraint, VersionConstraint) and result_constraint.is_simple()
    ):
        result_marker = SingleMarker(marker1.name, result_constraint)
    elif isinstance(result_constraint, UnionConstraint) and all(
        isinstance(c, Constraint) and c.operator == "==" for c in result_constraint.constraints
    ):
...
def _merge_python_version_single_markers(
    ...
    merged_marker = _merge_single_markers(normalized_marker, full_version_marker, merge_class)
