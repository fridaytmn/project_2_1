...
class PEP440Version:
    ...
    _compare_key: tuple[int, Release, ReleaseTag, ReleaseTag, ReleaseTag, tuple[int | str, ...]] = dataclasses.field(
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "text", self.to_string() if not self.text else str(self.text))
    ...
    def _make_compare_key(
        ...
        if self.local is None:
            ...
            _local = tuple((int(i), "") if str(i).isnumeric() else (NegativeInfinity(), i) for i in self.local)
    ...
    def next_minor(self: T) -> T:
        release = self.release
        if self._is_increment_required() or Release(release.major, release.minor, 0) < release:
            release = release.next_minor()
        return self.__class__(epoch=self.epoch, release=release)

    def next_patch(self: T) -> T:
        release = self.release
        if self._is_increment_required() or Release(release.major, release.minor, release.patch) < release:
    ...
    def replace(self: T, **kwargs: Any) -> T:
        return self.__class__(
            **{
                **{
                    k: getattr(self, k) for k in self.__dataclass_fields__ if k not in ("_compare_key", "text")
