    ...
    def expansion(self, tree: Tree):
        ...
        for i, child in enumerate(tree.children):
            if isinstance(child, Tree) and child.data == "expansions":
                ...
                tree.children = [
                    self.visit(ST("expansion", [option if i == j else other for j, other in enumerate(tree.children)]))
...
class Grammar:
    ...
    def compile(self, start, terminals_to_keep) -> Tuple[List[TerminalDef], List[Rule], List[str]]:
        ...
        while True:
            ...
            used_rules = {
                s for r in compiled_rules for s in r.expansion if isinstance(s, NonTerminal) and s != r.origin
            }
        ...
        if terminals_to_keep != "*":
            ...
            terminals, unused = classify_bool(
                terminals, lambda t: t.name in used_terms or t.name in self.ignore or t.name in terminals_to_keep
            )
            if unused:
...
def _make_rule_tuple(modifiers_tree, name, params, priority_tree, expansions):
    ...
    return (
        ...
        params,
        expansions,
        RuleOptions(keep_all_tokens, expand1, priority=priority, template_source=(name if params else None)),
    )
...
class GrammarBuilder:
    ...
    def _check_options(self, is_term, options):
        if is_term:
            if options is None:
                ...
                raise GrammarError(
                    "Terminal require a single int as 'options' (e.g. priority), got %s" % (type(options),)
                )
        else:
            if options is None:
                options = RuleOptions()
    ...
    def _extend(self, name, is_term, exp, params=(), options=None):
        ...
        if is_term != d.is_term:
            self._grammar_error(
                is_term, "Cannot extend {type} {name} - one is a terminal, while the other is not.", name
            )
    ...
    def load_grammar(
        self, grammar_text: str, grammar_name: str = "<?>", mangle: Optional[Callable[[str], str]] = None
    ) -> None:
        ...
        for source in to_try:
            try:
                ...
                for name in gb._definitions:
                    if name in self._definitions:
                        raise GrammarError(
                            "Cannot import '%s' from '%s': Symbol already defined." % (name, grammar_path)
                        )
        ...
        for name, d in self._definitions.items():
            ...
            for temp in exp.find_data("template_usage"):
                ...
                if sym not in params:
                    if sym not in self._definitions:
                        self._grammar_error(
                            d.is_term, "Template '%s' used but not defined (in {type} {name})" % sym, name
                        )
            ...
            for sym in _find_used_symbols(exp):
                if sym not in self._definitions and sym not in params:
                    self._grammar_error(
                        d.is_term, "{Type} '{name}' used but not defined (in {type2} {name2})", sym, name
                    )
