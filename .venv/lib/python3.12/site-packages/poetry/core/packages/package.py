...
AUTHOR_REGEX = re.compile(r"(?u)^(?P<name>[- .,\w\d'’\"():&]+)(?: <(?P<email>.+?)>)?$")  # noqa: RUF001
...
class Package(PackageSpecification):
    ...
    def __init__(
        ...
        if pretty_version is not None:
            warnings.warn(
                "The `pretty_version` parameter is deprecated and will be removed" " in a future release.",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def all_requires(
        ...
        """
        return [dependency for group in self._dependency_groups.values() for dependency in group.dependencies]
    ...
    def _get_maintainer(self) -> dict[str, str | None]:
        ...
        if m is None:
            raise ValueError("Invalid maintainer string. Must be in the format: " "John Smith <john@example.com>")
    ...
    def python_versions(self, value: str) -> None:
        ...
        self._python_marker = parse_marker(create_nested_marker("python_version", self._python_constraint))
    ...
    def all_classifiers(self) -> list[str]:
        ...
        for version in sorted(self.AVAILABLE_PYTHONS, key=lambda x: tuple(map(int, x.split(".")))):
            if len(version) == 1:
        ...
        for classifier in sorted(set(classifiers) - set(python_classifiers)):
            if not python_classifiers_inserted and classifier > python_classifier_prefix:
    ...
    def dependency_group_names(self, include_optional: bool = False) -> set[str]:
        return {name for name, group in self._dependency_groups.items() if not group.is_optional() or include_optional}
    ...
    def without_dependency_groups(self: T, groups: Collection[str]) -> T:
        ...
        """
        updated_groups = {
            group_name: group for group_name, group in self._dependency_groups.items() if group_name not in groups
        }
    ...
    def without_optional_dependency_groups(self: T) -> T:
        ...
        """
        updated_groups = {
            group_name: group for group_name, group in self._dependency_groups.items() if not group.is_optional()
        }
        package = self.clone()
        package._dependency_groups = updated_groups
    ...
    def with_dependency_groups(self: T, groups: Collection[str], only: bool = False) -> T:
        """
    ...
    def satisfies(self, dependency: Dependency, ignore_source_type: bool = False) -> bool:
        """
    ...
    def __repr__(self) -> str:
        ...
        if self._source_type:
            ...
            if self._source_resolved_reference:
                args.append(f"source_resolved_reference={self._source_resolved_reference!r}")
