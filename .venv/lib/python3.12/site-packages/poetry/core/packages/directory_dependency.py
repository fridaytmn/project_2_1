...
class DirectoryDependency(PathDependency):
    ...
    def _validate(self) -> str:
        ...
        if self._full_path.is_file():
            return f"{self._full_path} for {self.pretty_name} is a file," " expected a directory"
        if not is_python_project(self._full_path):
            return f"Directory {self._full_path} for {self.pretty_name} does not seem" " to be a Python package"
        return ""
