...
class Dependency(PackageSpecification):
    ...
    def python_versions(self, value: str) -> None:
        ...
        if not self._python_constraint.is_any():
            self._marker = self._marker.intersect(
                parse_marker(create_nested_marker("python_version", self._python_constraint))
            )
    ...
    def marker(self, marker: str | BaseMarker) -> None:
        ...
        if not contains_group_without_marker(markers, "python_version"):
            ...
            self._python_versions = normalize_python_version_markers(python_version_markers)
    ...
    def base_pep_508_name(self) -> str:
        ...
        if isinstance(constraint, VersionUnion):
            if constraint.excludes_single_version or constraint.excludes_single_wildcard_range:
                ...
                constraints = ",".join(str(parse_constraint(c)) for c in self.pretty_constraint.split(","))
    ...
    def to_pep_508(self, with_extras: bool = True, *, resolved: bool = False) -> str:
        ...
        if not self.marker.is_any():
            ...
            if self.python_versions != "*":
                ...
                markers.append(create_nested_marker("python_version", python_constraint))

        in_extras = " || ".join(self._in_extras)
        if in_extras and with_extras and not has_extras:
            markers.append(create_nested_marker("extra", parse_generic_constraint(in_extras)))
    ...
    @classmethod
    def create_from_pep_508(cls, name: str, relative_to: Path | None = None) -> Dependency:
        """
        ...
        if is_url(name):
            ...
            if os.path.isdir(p) and (os.path.sep in name or name.startswith(".")):
                if not is_python_project(Path(name)):
                    raise ValueError(f"Directory {name!r} is not installable. Not a Python project.")
        ...
        if link:
            ...
            if link.scheme.startswith("git+"):
                ...
                dep = VCSDependency(name, "git", link.url_without_fragment, extras=req.extras)
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return super().__eq__(other) and (self._constraint == other.constraint or self.is_direct_origin())
...
def _make_file_or_dir_dep(
    ...
    if _path.is_file():
        return FileDependency(name, path, base=base, directory=subdirectory, extras=extras)
    elif _path.is_dir():
        if subdirectory:
