...
def url_to_path(url: str) -> Path:
    ...
    if not netloc or netloc == "localhost":
        ...
        raise ValueError(f"non-local file URIs are not supported on this platform: {url}")
...
def convert_markers(marker: BaseMarker) -> ConvertedMarkers:
    ...
    def add_constraint(marker_name: str, constraint: tuple[str, str], group_index: int) -> None:
...
def create_nested_marker(
    ...
    if isinstance(constraint, (MultiConstraint, UnionConstraint)):
        ...
        if constraint.min is not None:
            ...
            if min_name == "python_version" and not constraint.include_min and version.precision < 3:
        ...
        if constraint.max is not None:
            ...
            if max_name == "python_version" and constraint.include_max and version.precision < 3:
                padding = ".0" * (3 - version.precision)
                part = f'python_full_version <= "{version}{padding}"'
            else:
