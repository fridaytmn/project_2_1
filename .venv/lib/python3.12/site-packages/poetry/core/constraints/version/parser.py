...
def _parse_constraint(constraints: str, *, is_marker_constraint: bool = False) -> VersionConstraint:
    ...
    for constraints in or_constraints:
        ...
        and_constraints = re.split(r"(?<!^)(?<![\^~=>< ,]) *(?<!-)[, ](?!-) *(?!,|$)", constraints)
        constraint_objects = []

        if len(and_constraints) > 1:
            for constraint in and_constraints:
                constraint_objects.append(
                    parse_single_constraint(constraint, is_marker_constraint=is_marker_constraint)
                )
        else:
            constraint_objects.append(
                parse_single_constraint(and_constraints[0], is_marker_constraint=is_marker_constraint)
            )
...
def parse_single_constraint(constraint: str, *, is_marker_constraint: bool = False) -> VersionConstraint:
    ...
    if m:
        try:
            ...
            raise ParseConstraintError(f"Could not parse version constraint: {constraint}") from e
