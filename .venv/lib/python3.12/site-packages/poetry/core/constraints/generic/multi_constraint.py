...
class MultiConstraint(BaseConstraint):
    def __init__(self, *constraints: Constraint) -> None:
        if any(c.operator == "==" for c in constraints):
            raise ValueError("A multi-constraint can only be comprised of negative constraints")
    ...
    def allows_any(self, other: BaseConstraint) -> bool:
        ...
        if isinstance(other, MultiConstraint):
            return any(c1.allows(c2) for c1 in self.constraints for c2 in other.constraints)
    ...
    def intersect(self, other: BaseConstraint) -> BaseConstraint:
        if isinstance(other, MultiConstraint):
            ...
            union = list(self.constraints) + [c for c in other.constraints if c not in ours]
