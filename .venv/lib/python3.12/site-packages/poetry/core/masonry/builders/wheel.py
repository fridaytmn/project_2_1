...
class WheelBuilder(Builder):
    ...
    def prepare_metadata(self, metadata_directory: Path) -> Path:
        ...
        if "scripts" in self._poetry.local_config or "plugins" in self._poetry.local_config:
            with (dist_info / "entry_points.txt").open("w", encoding="utf-8", newline="\n") as f:
...
""",
            ...
            raise RuntimeError(
                "Failed to get sys_tags for python interpreter" f" '{self.executable.as_posix()}':\n{e.output}"
            )
    ...
    def _write_to_zip(self, wheel: zipfile.ZipFile, rel_path: str) -> Iterator[StringIO]:
        sio = StringIO()
        yield sio
