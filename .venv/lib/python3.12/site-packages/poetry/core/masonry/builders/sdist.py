...
class SdistBuilder(Builder):
    ...
    def build(
        ...
        tar = tarfile.TarFile(target.as_posix(), mode="w", fileobj=gz, format=tarfile.PAX_FORMAT)
    ...
    def build_setup(self) -> bytes:
        ...
        if self._package.build_script:
            import_name = ".".join(Path(self._package.build_script).with_suffix("").parts)
            ...
            if isinstance(include, PackageInclude):
                if include.is_package():
                    ...
                    if pkg_dir is not None:
                        ...
                        if "" in package_dir:
                            package_dir.update((p, os.path.join(pkg_root, p.replace(".", "/"))) for p in _packages)
                        else:
        ...
        dependencies, extras = self.convert_dependencies(self._package, self._package.requires)
        if dependencies:
    ...
    def find_packages(self, include: PackageInclude) -> tuple[str | None, list[str], dict[str, list[str]]]:
        """
        ...
        for path, _dirnames, filenames in os.walk(str(base), topdown=True):
            ...
            is_subpkg = any(filename.endswith(".py") for filename in filenames) and not all(
            ...
            if is_subpkg:
                ...
                data_elements = [f.relative_to(self._path) for f in Path(path).glob("*") if not f.is_dir()]
    ...
    def find_files_to_add(self, exclude_build: bool = False) -> set[BuildIncludeFile]:
        ...
        for additional_file in additional_files:
            file = BuildIncludeFile(path=additional_file, project_root=self._path, source_root=self._path)
            if file.path.exists():
