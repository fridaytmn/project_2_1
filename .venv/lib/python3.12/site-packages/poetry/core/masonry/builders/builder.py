...
class Builder:
    ...
    def __init__(
        ...
        if not poetry.is_package_mode:
            raise RuntimeError("Building a package is not possible in non-package mode.")
    ...
    def _module(self) -> Module:
        ...
        for p in self._package.packages:
            ...
            if formats and self.format and self.format not in formats and not self._ignore_packages_formats:
                continue
        ...
        if self._excluded_files is None:
            ...
            for excluded_glob in self._package.exclude:
                for excluded in self._path.glob(str(excluded_glob)):
                    explicitly_excluded.add(Path(excluded).relative_to(self._path).as_posix())
            ...
            for inc in self._package.include:
                ...
                for included in self._path.glob(str(included_glob)):
                    explicitly_included.add(Path(included).relative_to(self._path).as_posix())
    ...
    def find_files_to_add(self, exclude_build: bool = True) -> set[BuildIncludeFile]:
        ...
        for include in self._module.includes:
            ...
            for file in include.elements:
                ...
                if isinstance(include, PackageInclude) and include.source and self.format == "wheel":
                    source_root = include.base
                else:
                    source_root = self._path

                if isinstance(include, PackageInclude) and include.target and self.format == "wheel":
                ...
                if self.is_excluded(include_file.relative_to_project_root()) and isinstance(include, PackageInclude):
                    continue
        ...
        for name, specification in self._poetry.local_config.get("scripts", {}).items():
            if isinstance(specification, dict) and specification.get("type") == "file":
                ...
                if Path(source).is_absolute():
                    raise RuntimeError(f"{source} in {name} is an absolute path. Expected relative" " path.")

                abs_path = Path.joinpath(self._path, source)

                if not abs_path.exists():
                    raise RuntimeError(f"{abs_path} in script specification ({name}) is not found.")

                if not abs_path.is_file():
                    raise RuntimeError(f"{abs_path} in script specification ({name}) is not a file.")
