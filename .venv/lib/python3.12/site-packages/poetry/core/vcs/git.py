...
def executable() -> str:
    ...
    if WINDOWS:
        ...
        paths = subprocess.check_output([where, "git"], shell=True, encoding="oem").split("\n")
        for path in paths:
            if not path:
                continue
...
class GitConfig:
    def __init__(self, requires_git_presence: bool = False) -> None:
        ...
        try:
            config_list = subprocess.check_output([executable(), "config", "-l"], stderr=subprocess.STDOUT).decode()
...
class Git:
    ...
    def normalize_url(cls, url: str) -> GitUrl:
        ...
        if parsed.subdirectory:
            formatted = re.sub(rf"[#&]subdirectory={parsed.subdirectory}$", "", formatted)

        altered = parsed.format() != formatted

        if altered:
            if re.match(r"^git\+https?", url) and re.match(r"^/?:[^0-9]", parsed.pathname or ""):
        ...
        return GitUrl(re.sub(r"#[^#]*$", "", normalized), parsed.rev, parsed.subdirectory)
    ...
    def remote_urls(self, folder: Path | None = None) -> dict[str, str]:
        output = self.run("config", "--get-regexp", r"remote\..*\.url", folder=folder).strip()
    ...
    def run(self, *args: Any, **kwargs: Any) -> str:
        ...
        return subprocess.check_output([executable(), *list(args)], stderr=subprocess.STDOUT).decode().strip()
