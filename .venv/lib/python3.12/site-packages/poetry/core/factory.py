...
class Factory:
    ...
    def create_poetry(self, cwd: Path | None = None, with_groups: bool = True) -> Poetry:
        from poetry.core.poetry import Poetry
        ...
        package = self.configure_package(package, local_config, poetry_file.parent, with_groups=with_groups)
    ...
    def _add_package_group_dependencies(
        ...
        for name, constraints in dependencies.items():
            _constraints = constraints if isinstance(constraints, list) else [constraints]
    ...
    def configure_package(
        ...
        package.authors = [combine_unicode(author) for author in config.get("authors", [])]

        package.maintainers = [combine_unicode(maintainer) for maintainer in config.get("maintainers", [])]
        ...
        if "dependencies" in config:
            cls._add_package_group_dependencies(package=package, group=MAIN_GROUP, dependencies=config["dependencies"])

        if with_groups and "group" in config:
            for group_name, group_config in config["group"].items():
                group = DependencyGroup(group_name, optional=group_config.get("optional", False))
        ...
        if with_groups and "dev-dependencies" in config:
            cls._add_package_group_dependencies(package=package, group="dev", dependencies=config["dev-dependencies"])
    ...
    def create_dependency(
        ...
        if isinstance(constraint, dict):
            ...
            allows_prereleases = constraint.get("allow-prereleases", constraint.get("allows-prereleases", False))
            ...
            if python_versions:
                marker = marker.intersect(
                    parse_marker(create_nested_marker("python_version", parse_version_constraint(python_versions)))
                )

            if platform:
                marker = marker.intersect(
                    parse_marker(create_nested_marker("sys_platform", parse_generic_constraint(platform)))
                )
        ...
        if strict:
            ...
            if "dependencies" in config:
                ...
                if python_versions == "*":
                    result["warnings"].append(
                        "A wildcard Python dependency is ambiguous. " "Consider specifying a more explicit one."
                    )
                    ...
                    for req in requirements:
                        ...
                        for dependency in config.get("dependencies", {}):
                            ...
                            result["errors"].append(
                                f'Cannot find dependency "{req}" for extra ' f'"{extra_name}" in main dependencies.'
                            )
                ...
                for name, script in scripts.items():
                    ...
                    for extra in extras:
                        if extra not in config_extras:
                            result["errors"].append(
                                f'The script "{name}" requires extra "{extra}"' " which is not defined."
                            )
            ...
            if "readme" in config and not isinstance(config["readme"], str):
                ...
                if len(readme_types) > 1:
                    result["errors"].append(
                        "Declared README files must be of same type: found" f" {', '.join(sorted(readme_types))}"
                    )
    ...
    def locate(cls, cwd: Path | None = None) -> Path:
        ...
        for path in candidates:
            ...
            raise RuntimeError(f"Poetry could not find a pyproject.toml file in {cwd} or its parents")
