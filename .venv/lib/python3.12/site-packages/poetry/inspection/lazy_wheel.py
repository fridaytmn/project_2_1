...
class MergeIntervals:
    ...
    def __repr__(self) -> str:
        return f"{type(self).__name__}" f"(left={tuple(self._left)}, right={tuple(self._right)})"
    ...
    def minimal_intervals_covering(self, start: int, end: int) -> Iterator[tuple[int, int]]:
...
class LazyFileOverHTTP(ReadOnlyIOWrapper):
    ...
    def _reset_content(self) -> None:
        ...
        if self._merge_intervals is not None:
            logger.debug("unsetting merge intervals (were: %s)", repr(self._merge_intervals))
    ...
    def _content_length_from_head(self) -> int:
        ...
        if accepted_range != "bytes":
            raise HTTPRangeRequestUnsupported(f"server does not support byte ranges: header was '{accepted_range}'")
...
class LazyWheelOverHTTP(LazyFileOverHTTP):
        ...
        if tail is None:
            ...
            with self._stay():
                ...
                assert ((init_chunk_start, init_chunk_end),) == tuple(
                    ...
                    # been covered.
                    self._merge_intervals.minimal_intervals_covering(init_chunk_start, init_chunk_end)
                )
        return ret_length
    ...
    def _try_initial_chunk_request(self, initial_chunk_size: int) -> tuple[int, Response]:
        ...
        if code != codes.partial_content:
            ...
            raise HTTPRangeRequestUnsupported(f"did not receive partial content: got code {code}")
        ...
        file_length = self._parse_full_length_from_content_range(tail.headers["Content-Range"])
        return (file_length, tail)

    def _extract_content_length(self, initial_chunk_size: int) -> tuple[int, Response | None]:
        ...
        try:
            ...
            if code == codes.requested_range_not_satisfiable and resp is not None and "Content-Range" in resp.headers:
                ...
                file_length = self._parse_full_length_from_content_range(resp.headers["Content-Range"])
        ...
        if int(tail.headers["Content-Length"]) > initial_chunk_size or tail.headers.get("Content-Range", "").startswith(
            "bytes -"
        ):
            tail = None
            self._domains_without_negative_range.add(domain)
        ...
        for info in zf.infolist():
            if start is None:
                ...
                if not self._metadata_regex.search(info.filename):
                    ...
                    break
        if start is None:
            raise UnsupportedWheel(f"no {self._metadata_regex!r} found for {name} in {self.name}")
