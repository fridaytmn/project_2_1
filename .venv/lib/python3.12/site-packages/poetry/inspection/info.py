...
class PackageInfo:
    ...
    @classmethod
    def _from_distribution(cls, dist: pkginfo.BDist | pkginfo.SDist | pkginfo.Wheel) -> PackageInfo:
        """
    ...
    def from_setup_files(cls, path: Path) -> PackageInfo:
        ...
        if not cls.has_setup_files(path):
            raise PackageInfoError(path, "No setup files (setup.py, setup.cfg) was found.")
...
def get_pep517_metadata(path: Path) -> PackageInfo:
    ...
    with ephemeral_environment(flags={"no-pip": False, "setuptools": "bundle", "wheel": "bundle"}) as venv:
        # TODO: cache PEP 517 build environment corresponding to each project venv
        dest_dir = venv.path.parent / "dist"
        dest_dir.mkdir()

        pep517_meta_build_script = PEP517_META_BUILD.format(source=path.as_posix(), dest=dest_dir.as_posix())
        ...
        try:
            ...
            try:
                ...
                raise PackageInfoError(path, e, "Fallback egg_info generation failed.", fbe)
            finally:
                os.chdir(cwd)
