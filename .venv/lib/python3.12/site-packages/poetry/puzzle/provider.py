...
class Provider:
    ...
    def search_for_installed_packages(
        ...
        packages = [
            package for package in self._installed_packages if package.satisfies(dependency, ignore_source_type=True)
        ]
        logger.debug(
    ...
    def _search_for_vcs(self, dependency: VCSDependency) -> Package:
        ...
        package = self._direct_origin.get_package_from_vcs(
            ...
            source_root=self._source_root or (self._env.path.joinpath("src") if self._env else None),
        )
    ...
    def _get_dependencies_with_overrides(self, dependencies: list[Dependency], package: Package) -> list[Dependency]:
        ...
        if package.is_root():
            ...
            if not package.python_constraint.allows_all(self._python_constraint):
                ...
                if (
                    ...
                    or self._python_constraint.intersect(dependency_package.dependency.python_constraint).is_empty()
                    ...
                    return [
                        Incompatibility(
                            ...
                            PythonCause(package.python_versions, str(self._python_constraint)),
                        )
                    ]
    ...
    def complete_package(self, dependency_package: DependencyPackage) -> DependencyPackage:
            ...
            try:
                ...
                try:
                    dependency_package = next(
                        DependencyPackage(dependency, pkg) for pkg in self.search_for_installed_packages(dependency)
                    )
        ...
        for dep in requires:
            ...
            if not package.is_root() and (
                ...
                or (dep.in_extras and not set(dep.in_extras).intersection(dependency.extras))
            ):
                continue
        ...
        for dep in dependencies:
            if not dependency.transitive_marker.without_extras().is_any():
                transitive_marker_intersection = dependency.transitive_marker.without_extras().intersect(
    ...
    def debug(self, message: str, depth: int = 0) -> None:
        ...
        if message.startswith("fact:"):
            if "depends on" in message:
                ...
                message = re.sub(r"(?<=: )(.+?) \((.+?)\)", "<c1>\\1</c1> (<c2>\\2</c2>)", message)
            ...
            if m:
                message = f"<fg=blue>derived</>: <c1>{m.group(1)}</c1>" f" (<c2>{m.group(2)}</c2>)"
            else:
            ...
            if m:
                ...
                message = "<fg=red;options=bold>conflict</>:" f" {message.split('conflict: ')[1]}"
    ...
    def _is_relevant_marker(self, marker: BaseMarker, active_extras: Collection[NormalizedName] | None) -> bool:
        ...
        return (
            not marker.is_empty()
            and self._python_constraint.allows_any(get_python_constraint_from_marker(marker))
    ...
    def _resolve_overlapping_markers(
        ...
        for uses in itertools.product([True, False], repeat=len(dependencies)):
            ...
            markers = (
                ...
                for use, dep in sorted(zip(uses, dependencies), key=lambda ud: ud[0], reverse=True)
            )
            ...
            # (and correct source)
            new_dep = (
                specific_source_dependency if specific_source_dependency else used_dependencies[0]
