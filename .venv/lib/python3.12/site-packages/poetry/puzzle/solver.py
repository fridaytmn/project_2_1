...
class Solver:
    def __init__(
        ...
        self._provider = Provider(self._package, self._pool, self._io, installed=installed, locked=locked)
    ...
    def solve(self, use_latest: Collection[NormalizedName] | None = None) -> Transaction:
        from poetry.puzzle.transaction import Transaction
        ...
        for p in packages:
            if p.yanked:
                message = f"The locked version {p.pretty_version} for {p.pretty_name} is a" " yanked version."
                if p.yanked_reason:
    ...
    def _progress(self) -> Iterator[None]:
        if not self._io.output.is_decorated() or self._provider.is_debugging():
            ...
            indicator = Indicator(self._io, "{message}{context}<debug>({elapsed:2s})</debug>")
        ...
        for package in packages:
            if package.features:
                for _package in packages:
                    if not _package.features and _package.name == package.name and _package.version == package.version:
                        for dep in package.requires:
...
def depth_first_search(source: PackageNode) -> list[list[PackageNode]]:
    ...
    combined_topo_sorted_nodes: list[list[PackageNode]] = [
        combined_nodes.pop(node.name) for node in topo_sorted_nodes if node.name in combined_nodes
    ]
...
class PackageNode(DFSNode):
    ...
    def reachable(self) -> list[PackageNode]:
        ...
        for dependency in self.package.all_requires:
            for pkg in self.packages:
                if pkg.complete_name == dependency.complete_name and (dependency.constraint.allows(pkg.version)):
                    children.append(
                        PackageNode(
                            pkg,
                            self.packages,
    ...
    def visit(self, parents: list[PackageNode]) -> None:
        ...
        self.depth = 1 + max(
            [parent.depth if parent.base_name != self.base_name else parent.depth - 1 for parent in parents] + [-2]
        )
