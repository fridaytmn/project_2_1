...
def get_package_version_display_string(package: Package, root: Path | None = None) -> str:
...
def get_highest_priority_hash_type(hash_types: set[str], archive_name: str) -> str | None:
    if not hash_types:
        return None
    ...
    logger.debug(
        f"There are no known hash types for {archive_name} that are prioritised (known" f" hash types: {hash_types!s})"
    )
...
def extractall(source: Path, dest: Path, zip: bool) -> None:
    ...
    if zip:
        ...
        with tarfile.open(source) as archive:
            if hasattr(tarfile, "data_filter") and sys.version_info[:3] not in broken_tarfile_filter:
