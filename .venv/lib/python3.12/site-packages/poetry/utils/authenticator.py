...
class RepositoryCertificateConfig:
    ...
    @classmethod
    def create(cls, repository: str, config: Config | None) -> RepositoryCertificateConfig:
...
class AuthenticatorRepositoryConfig:
    ...
    def get_http_credentials(
        ...
        credential = HTTPAuthCredential(**(password_manager.get_http_auth(self.name) or {}))
        ...
        if password_manager.use_keyring:
            ...
            credential = password_manager.get_credential(self.url, self.netloc, username=credential.username)
...
class Authenticator:
    def __init__(
        ...
        self._configured_repositories: dict[str, AuthenticatorRepositoryConfig] | None = None
        self._password_manager = PasswordManager(self._config)
        self._cache_control = (
            FileCache(self._config.repository_cache_directory / (cache_id or "_default_cache") / "_http")
            if not disable_cache
            else None
        )
        self.get_repository_config_for_url = functools.lru_cache(maxsize=None)(self._get_repository_config_for_url)
    ...
    def request(self, method: str, url: str, raise_for_status: bool = True, **kwargs: Any) -> requests.Response:
        ...
        if credential.username is not None or credential.password is not None:
            request = requests.auth.HTTPBasicAuth(credential.username or "", credential.password or "")(request)
        ...
        settings = session.merge_environment_settings(prepared_request.url, proxies, stream, verify, cert)
    ...
    def _get_credentials_for_url(self, url: str, exact_match: bool = False) -> HTTPAuthCredential:
        ...
        if credential.password is None:
            parsed_url = urllib.parse.urlsplit(url)
            netloc = parsed_url.netloc
            credential = self._password_manager.get_credential(url, netloc, username=credential.username)
    ...
    def get_http_auth(self, name: str, username: str | None = None) -> HTTPAuthCredential | None:
        if name == "pypi":
            repository = AuthenticatorRepositoryConfig(name, "https://upload.pypi.org/legacy/")
        else:
            if name not in self.configured_repositories:
                return None
            repository = self.configured_repositories[name]

        return self._get_credentials_for_repository(repository=repository, username=username)
    ...
    def configured_repositories(self) -> dict[str, AuthenticatorRepositoryConfig]:
        if self._configured_repositories is None:
            ...
            for repository_name in self._config.get("repositories", []):
                ...
                self._configured_repositories[repository_name] = AuthenticatorRepositoryConfig(repository_name, url)
    ...
    def _get_repository_config_for_url(
        ...
        for repository in self.configured_repositories.values():
            ...
            if repository.netloc == parsed_url.netloc:
                if parsed_url.path.startswith(repository.path) or commonprefix((parsed_url.path, repository.path)):
        ...
        if len(candidates) > 1:
            ...
            candidates.sort(key=lambda c: len(commonprefix([parsed_url.path, c.path])), reverse=True)
