...
class PasswordManager:
    ...
    def keyring(self) -> PoetryKeyring:
        if not self.use_keyring:
            raise PoetryKeyringError("Access to keyring was requested, but it is not available")
    ...
    def set_pypi_token(self, repo_name: str, token: str) -> None:
        if not self.use_keyring:
            ...
            self._config.auth_config_source.add_property(f"pypi-token.{repo_name}", token)
        else:
    ...
    def delete_pypi_token(self, repo_name: str) -> None:
        if not self.use_keyring:
            return self._config.auth_config_source.remove_property(f"pypi-token.{repo_name}")
