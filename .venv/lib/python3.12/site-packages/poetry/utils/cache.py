...
class ArtifactCache:
    ...
    def get_cache_directory_for_link(self, link: Link) -> Path:
        ...
        if hash_name := get_highest_priority_hash_type(set(link.hashes.keys()), link.filename):
    ...
    def _get_directory_from_hash(self, key_parts: object) -> Path:
        key = hashlib.sha256(
            json.dumps(key_parts, sort_keys=True, separators=(",", ":"), ensure_ascii=True).encode("ascii")
        ).hexdigest()
