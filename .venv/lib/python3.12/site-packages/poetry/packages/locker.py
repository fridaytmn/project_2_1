...
GENERATED_COMMENT = (
    f"This file is automatically {_GENERATED_IDENTIFIER} by Poetry" f" {__version__} and should not be changed by hand."
)
...
class Locker:
    ...
    def _compute_lock_data(self, root: Package, packages: list[Package]) -> TOMLDocument:
        ...
        if root.extras:
            lock["extras"] = {
                extra: sorted(dep.pretty_name for dep in deps) for extra, deps in sorted(root.extras.items())
            }
