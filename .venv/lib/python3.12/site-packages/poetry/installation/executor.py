...
class Executor:
    def __init__(
        ...
        self._use_modern_installation = config.get("installer.modern-installation", True)
        if not self._use_modern_installation:
            self._io.write_line(
                "<warning>Warning: Setting `installer.modern-installation` to `false` " "is deprecated.</>"
            )
            self._io.write_line("<warning>The pip-based installer will be removed in a future release.</>")
            self._io.write_line("<warning>See https://github.com/python-poetry/poetry/issues/8987.</>")
        ...
        self._authenticator = Authenticator(config, self._io, disable_cache=disable_cache, pool_size=self._max_workers)
    ...
    def pip_install(self, req: Path, upgrade: bool = False, editable: bool = False) -> int:
        try:
            pip_install(req, self._env, upgrade=upgrade, editable=editable)
        except EnvCommandError as e:
            output = decode(e.e.output)
            if "KeyboardInterrupt" in output or "ERROR: Operation cancelled by user" in output:
                return -2
            raise
        ...
        for _, group in groups:
            ...
            for operation in group:
                ...
                is_parallel_unsafe = operation.job_type == "uninstall" or (
                    operation.package.develop and operation.package.source_type in {"directory", "git"}
                )
    ...
    def _write(self, operation: Operation, line: str) -> None:
        if not self.supports_fancy_output() or not self._should_write_operation(operation):
            return
    ...
    def _execute_operation(self, operation: Operation) -> None:
        try:
            ...
            if self.supports_fancy_output():
                if id(operation) not in self._sections and self._should_write_operation(operation):
                    with self._lock:
                ...
                if self._should_write_operation(operation):
                    if not operation.skipped:
                        self._io.write_line(f"  <fg=blue;options=bold>-</> {op_message}")
                    else:
                        self._io.write_line(
                ...
                with self._lock:
                    ...
                    if isinstance(e, ChefBuildError):
                        ...
                        if pkg.develop:
                            ...
                            requirement = pkg.to_dependency().to_pep_508().split(";")[0].strip()
                        message = (
                            "<info>"
                        ...
                        message = "<error>" "Cannot install build-system.requires" f" for {pkg.pretty_name}." "</error>"
                    elif isinstance(e, SolverProblemError):
                        message = "<error>" "Cannot resolve build-system.requires" f" for {pkg.pretty_name}." "</error>"
                    else:
    ...
    def get_operation_message(
        ...
        if isinstance(operation, Update):
            ...
            update_kind = "Updating" if target_version >= initial_version else "Downgrading"
            return (
    ...
    def _prepare_archive(self, operation: Install | Update, *, output_dir: Path | None = None) -> Path:
        package = operation.package
        operation_message = self.get_operation_message(operation)
        ...
        return self._chef.prepare(archive, editable=package.develop, output_dir=output_dir)
    ...
    def _install_directory_without_wheel_installer(self, operation: Install | Update) -> int:
    ...
    def _download_link(self, operation: Install | Update, link: Link) -> Path:
        ...
        if archive is None:
            ...
            # current environment.
            raise RuntimeError(
                f"Package {link.url} cannot be installed in the current environment" f" {self._env.marker_env}"
            )

        if archive.suffix != ".whl":
            message = (
                f"  <fg=blue;options=bold>-</> {self.get_operation_message(operation)}:" " <info>Preparing...</info>"
            )
            self._write(operation, message)
    ...
    def _validate_archive_hash(archive: Path, package: Package) -> str:
        ...
        if archive_hash not in known_hashes:
            raise RuntimeError(
                f"Hash for {package} from archive {archive.name} not found in" f" known hashes (was: {archive_hash})"
            )
    ...
    def _download_archive(
        ...
        if self.supports_fancy_output():
            if wheel_size is None:
                ...
                progress = ProgressBar(self._sections[id(operation)], max=int(wheel_size))
    ...
    def _should_write_operation(self, operation: Operation) -> bool:
        return not operation.skipped or self._dry_run or self._verbose or not self._enabled
    ...
    def _save_url_reference(self, operation: Operation) -> None:
        ...
        if not package.source_url or package.source_type == "legacy":
            if not self._use_modern_installation:
                ...
                # exists.
                for direct_url_json in self._env.site_packages.find_distribution_direct_url_json_files(
        ...
        if url_reference:
            for dist in self._env.site_packages.distributions(name=package.name, writable_only=True):
