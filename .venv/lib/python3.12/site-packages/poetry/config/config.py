...
class PackageFilterPolicy:
    ...
    def allows(self, package_name: str) -> bool:
        ...
        return not self.packages or ":none:" in self.packages or canonicalize_name(package_name) not in self.packages
    ...
    def validator(cls, policy: str) -> bool:
        ...
        for name in names:
            if not name or (cls.is_reserved(name) and len(names) == 1) or re.match(r"^[a-zA-Z\d_-]+$", name):
                continue
            return False
...
class Config:
    ...
    def __init__(self, use_environment: bool = True, base_dir: Path | None = None) -> None:
