...
class EditableBuilder(Builder):
    ...
    def build(self, target_dir: Path | None = None) -> Path:
        ...
        for removed in self._env.site_packages.remove_distribution_files(distribution_name=self._package.name):
            self._debug(f"  - Removed <c2>{removed.name}</c2> directory from" f" <b>{removed.parent}</b>")
    ...
    def _add_pth(self) -> list[Path]:
        paths = {
            ...
            if isinstance(include, PackageInclude) and (include.is_module() or include.is_package())
        }
        ...
        try:
            ...
            self._io.write_error_line(
                f"  - Failed to create <c2>{pth_file.name}</c2> for" f" {self._poetry.file.path.parent}"
            )
            return []
    ...
    def _add_scripts(self) -> list[Path]:
        ...
        for scripts_path in self._env.script_dirs:
            ...
            self._io.write_error_line(
                "  - Failed to find a suitable script installation directory for" f" {self._poetry.file.path.parent}"
            )
            return []
        ...
        for script in scripts:
            ...
            if WINDOWS:
                ...
                self._debug(f"  - Adding the <c2>{cmd_script.name}</c2> script wrapper to" f" <b>{scripts_path}</b>")
    ...
    def _add_dist_info(self, added_files: list[Path]) -> None:
        ...
        self._debug(f"  - Adding the <c2>{dist_info.name}</c2> directory to" f" <b>{dist_info.parent}</b>")
