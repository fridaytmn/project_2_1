...
class Incompatibility:
    ...
    def __str__(self) -> str:
        if isinstance(self._cause, DependencyCause):
            ...
            return f"{self._terse(depender, allow_every=True)} depends on" f" {self._terse(dependee)}"
            ...
            return f"no versions of {self._terms[0].dependency.name} match" f" {self._terms[0].constraint}"
        elif isinstance(self._cause, RootCause):
            assert len(self._terms) == 1
            assert not self._terms[0].is_positive()
            assert self._terms[0].dependency.is_root

            return f"{self._terms[0].dependency.name} is" f" {self._terms[0].dependency.constraint}"
        ...
        if positive and negative:
            ...
            return f"{self._terse(positive_term, allow_every=True)} requires" f" {' or '.join(negative)}"
        elif positive:
