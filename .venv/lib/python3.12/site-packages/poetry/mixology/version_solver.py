...
class DependencyCache:
    ...
    def search_for(
        ...
        packages = self._search_for_cached(dependency.without_features() if dependency.features else dependency, key)
...
class VersionSolver:
    ...
    def _resolve_conflict(self, incompatibility: Incompatibility) -> Incompatibility:
        ...
        while not incompatibility.is_failure():
            ...
            self._log(f"! {most_recent_term} is{partially} satisfied by" f" {most_recent_satisfier}")
    ...
    def _choose_package_version(self) -> str | None:
        ...
        if not conflict:
            ...
            self._log(f"selecting {package.package.complete_name}" f" ({package.package.full_pretty_version})")
