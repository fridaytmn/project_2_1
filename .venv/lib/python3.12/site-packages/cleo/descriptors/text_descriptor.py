...
class TextDescriptor(Descriptor):
    def _describe_argument(self, argument: Argument, **options: Any) -> None:
        if argument.default is not None and (not isinstance(argument.default, list) or argument.default):
            default = f"<comment> [default: {self._format_default_value(argument.default)}]" "</comment>"
        else:
            default = ""
        ...
        self._write(f"  <c1>{argument.name}</c1>  {' ' * spacing_width}" f"{sub_argument_description}{default}")
    ...
    def _describe_option(self, option: Option, **options: Any) -> None:
        if (
            ...
            default = "<comment> [default: " f"{self._format_default_value(option.default)}]</comment>"
        else:
            default = ""
        ...
        are_multiple_values_allowed = "<comment> (multiple values allowed)</comment>" if option.is_list() else ""
        self._write(
            f"  <c1>{synopsis}</c1>  "
    ...
    def _describe_application(self, application: Application, **options: Any) -> None:
        ...
        if described_namespace:
            self._write(f'<b>Available commands for the "{described_namespace}" namespace:</b>')
        else:
        ...
        for namespace in namespaces.values():
            ...
            if not (described_namespace or namespace["id"] == ApplicationDescription.GLOBAL_NAMESPACE):
                self._write("\n")
            ...
            for name in namespace["commands"]:
                ...
                command_aliases = self._get_command_aliases_text(command) if command.name == name else ""
                self._write(f"  <c1>{name}</c1>{' ' * spacing_width}" f"{command_aliases + command.description}")
    ...
    def _format_default_value(self, default: Any) -> str:
        if isinstance(default, str):
            ...
            default = [Formatter.escape(value) for value in default if isinstance(value, str)]
        elif isinstance(default, dict):
            default = {key: Formatter.escape(value) for key, value in default.items() if isinstance(value, str)}
