...
class Color:
    ...
    def __init__(
        ...
        for option in options or []:
            if option not in self.AVAILABLE_OPTIONS:
                raise ValueError(
                    f'"{option}" is not a valid color option. ' f"It must be one of {', '.join(self.AVAILABLE_OPTIONS)}"
                )
    ...
    def _parse_color(self, color: str, background: bool) -> str:
        ...
        if color.startswith("#"):
            ...
            return ("4" if background else "3") + self._convert_hex_color_to_ansi(int(color, 16))

        if color not in self.COLORS:
            raise CleoValueError(f'"{color}" is an invalid color.' f" It must be one of {', '.join(self.COLORS)}")
