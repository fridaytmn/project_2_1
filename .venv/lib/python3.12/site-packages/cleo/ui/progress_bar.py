...
class ProgressBar(Component):
    ...
    def set_progress(self, step: int) -> None:
        ...
        redraw_freq = (self._max or 10) / 10 if self.redraw_freq is None else self.redraw_freq
        ...
        if prev_period != curr_period or time_interval >= self._max_seconds_between_redraws:
            self.display()
        ...
        if self._should_overwrite:
            if self._previous_message is not None:
                if isinstance(self._io, SectionOutput):
                    lines_to_clear = (
                        len(self._io.remove_format(message)) // self._terminal.width + self._format_line_count + 1
                    )
    ...
    def bar_offset(self) -> int:
        ...
        if self.redraw_freq is None:
            return math.floor((min(5, self.bar_width // 15) * self._write_count) % self.bar_width)
    ...
    def _formatter_bar(self) -> str:
        ...
        if complete_bars < self.bar_width:
            empty_bars = self.bar_width - complete_bars - len(self._io.remove_format(self.progress_char))
    ...
    def _formatter_remaining(self) -> str:
        if not self._max:
            raise RuntimeError("Unable to display the remaining time " "if the maximum number of steps is not set.")

        if not self._step:
            remaining = 0
        else:
            remaining = round((time.time() - self._start_time) / self._step * (self._max - self._max))

        return format_time(remaining)

    def _formatter_estimated(self) -> int:
        if not self._max:
            raise RuntimeError("Unable to display the estimated time " "if the maximum number of steps is not set.")
    ...
    def _build_line(self) -> str:
        ...
        lines_length = [len(self._io.remove_format(sub_line.rstrip("\r"))) for sub_line in line.split("\n")]
