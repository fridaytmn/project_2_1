...
class ConfirmationQuestion(Question):
    ...
    def __init__(self, question: str, default: bool = True, true_answer_regex: str = r"(?i)^y") -> None:
    ...
    def _write_prompt(self, io: IO) -> None:
        message = f"<question>{self._question} (yes/no)</> " f'[<comment>{"yes" if self._default else "no"}</>] '
