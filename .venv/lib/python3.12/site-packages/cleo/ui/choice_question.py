...
class ChoiceQuestion(Question):
    ...
    def __init__(self, question: str, choices: list[str], default: Any | None = None) -> None:
    ...
    def _write_prompt(self, io: IO) -> None:
        ...
        if default is None:
            ...
            message = f"<question>{message}</question> " f"[<comment>{', '.join(default)}</comment>]:"
        else:
            choices = self._choices
            message = f"<question>{message}</question> " f"[<comment>{choices[int(default)]}</comment>]:"
