...
class Highlighter:
    ...
    def code_snippet(self, source: str, line: int, lines_before: int = 2, lines_after: int = 2) -> list[str]:
    ...
    def split_to_lines(self, source: str) -> list[str]:
        ...
        def readline() -> bytes:
            return formatter.format(formatter.escape(source_io.readline().decode())).encode()
    ...
    def line_numbers(self, lines: list[str], mark_line: int | None = None) -> list[str]:
        ...
        for i, line in enumerate(lines):
            ...
            snippet += (
                f"<{styling}>" f"{line_number}</><{self._theme[self.LINE_NUMBER]}>" f"{self._ui['delimiter']}</> {line}"
            )
...
class ExceptionTrace:
    ...
    def _render_exception(self, io: IO | Output, exception: BaseException) -> None:
        ...
        if inspector.has_previous_exception():
            ...
            io.write_line("")
            io.write_line("The following error occurred when trying to handle this error:")
            io.write_line("")
        ...
        io.write_line("")
        exception_message = Formatter().format(inspector.exception_message).replace("\n", "\n  ")
    ...
    def _render_snippet(self, io: IO | Output, frame: Frame) -> None:
        ...
        code_lines = Highlighter(supports_utf8=io.supports_utf8()).code_snippet(frame.file_content, frame.lineno, 4, 4)
    ...
    def _render_solution(self, io: IO | Output, exception: Exception) -> None:
        ...
        solutions = self._solution_provider_repository.get_solutions_for_exception(exception)
    ...
    def _render_trace(self, io: IO | Output, frames: FrameCollection) -> None:
        ...
        for frame in frames:
            if self._ignore and re.match(self._ignore, frame.filename) and not io.is_debug():
                continue
            ...
            for collection in frame_collections:
                ...
                for frame in collection:
                    ...
                    relative_file_path = f"<fg=default;options=dark>{Formatter.escape(os.sep)}</>".join(
                        ...
                        + ["<fg=default;options=bold>" f"{relative_file_path_parts[-1]}</>"]
                    )
                    self._render_line(
                        io,
                    ...
                    if io.is_debug():
                        ...
                        try:
                            code_line = highlighter.highlighted_lines(frame.line.strip())[0]
                        except tokenize.TokenError:
                            code_line = frame.line.strip()

                        self._render_line(io, f"{' ' * (max_frame_length + 4)}{code_line}")

                    i -= 1

    def _render_line(self, io: IO | Output, line: str, new_line: bool = False, indent: int = 2) -> None:
        if new_line:
            io.write_line("")
