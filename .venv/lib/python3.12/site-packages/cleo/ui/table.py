...
class Table:
    ...
    def render(self) -> None:
        ...
        for row in rows:
            ...
            if self._horizontal:
                self._render_row(row, self.style.cell_row_format, self.style.cell_header_format)
            else:
                self._render_row(
                    row,
                    self.style.cell_header_format if is_header else self.style.cell_row_format,
                )
    ...
    def _render_row_separator(
        ...
        if title is not None:
            ...
            if title_length > limit:
                ...
                formatted_title = title_format.format(title[: limit - format_length - 3] + "...")

            title_start = (markup_length - title_length) // 2
            markup = markup[:title_start] + formatted_title + markup[title_start + title_length :]
    ...
    def _render_column_separator(self, type: int = BORDER_OUTSIDE) -> str:
        ...
        return self.style.border_format.format(borders[1] if type == self.BORDER_OUTSIDE else borders[3])

    def _render_row(self, row: list[str], cell_format: str, first_cell_format: str | None = None) -> None:
        """
        Renders table row.
        ...
        for i, column in enumerate(columns):
            ...
            row_content += self._render_column_separator(self.BORDER_OUTSIDE if i == last else self.BORDER_INSIDE)
    ...
    def _render_cell(self, row: Row, column: int, cell_format: str) -> str:
        ...
        if isinstance(cell, TableCell) and cell.colspan > 1:
            ...
            for next_column in range(column + 1, column + cell.colspan):
                width += self._get_column_separator_width() + self._effective_column_widths[next_column]
        ...
        if isinstance(cell, TableCell) and isinstance(cell.style, TableCellStyle):
            is_not_styled_by_tag = not re.match(
                (r"^<(\w+|((?:fg|bg|options)=[\w,]+;?)+)>" r".+<\/(\w+|((?:fg|bg|options)=[\w,]+;?)+)?>$"),
                str(cell),
            )
            if is_not_styled_by_tag:
                cell_format = (
                    cell.style.cell_format if cell.style.cell_format is not None else f"<{cell.style.tag}>{{}}</>"
                )
    ...
    def _build_table_rows(self, rows: Rows) -> Iterator[Row | TableSeparator]:
        ...
        while row_key < len(rows):
            ...
            for column, cell in enumerate(rows[row_key]):
                ...
                if column in self._column_max_widths and self._column_max_widths[column] < len(
                    self._io.remove_format(cell)
                ):
                    assert isinstance(self._io, Output)
                    cell = self._io.formatter.format_and_wrap(cell, self._column_max_widths[column] * colspan)

                if "\n" not in cell:
                    continue

                escaped = "\n".join(Formatter.escape_trailing_backslash(c) for c in cell.split("\n"))
                cell = TableCell(escaped, colspan=cell.colspan) if isinstance(cell, TableCell) else escaped
                ...
                for line_key, line in enumerate(lines):
                    ...
                    if line_key == 0:
                        ...
                        if line_key not in unmerged_rows[row_key]:
                            unmerged_rows[row_key][line_key] = self._copy_row(rows, row_key)
    ...
    def _calculate_row_count(self) -> int:
        number_of_rows = len(
            list(self._build_table_rows([*cast("Rows", self._headers), TableSeparator(), *self._rows]))
        )
        ...
        for column, cell in enumerate(rows[line]):
            if isinstance(cell, TableCell) and cell.rowspan > 1:
                ...
                if "\n" in cell:
                    ...
                    row[column] = TableCell(lines[0], colspan=cell.colspan, style=cell.style)
                ...
                for unmerged_row_key, _ in unmerged_rows.items():
                    ...
                    unmerged_rows[unmerged_row_key][column] = TableCell(value, colspan=cell.colspan, style=cell.style)
    ...
    def _get_row_columns(self, row: Row) -> list[int]:
        ...
        for cell_key, cell in enumerate(row):
            if isinstance(cell, TableCell) and cell.colspan > 1:
                ...
                columns = [column for column in columns if column not in range(cell_key + 1, cell_key + cell.colspan)]
    ...
    def _calculate_column_widths(self, rows: Rows) -> None:
        ...
        for column in range(self._number_of_columns):
                ...
                for i, cell in enumerate(row_):
                    if isinstance(cell, TableCell):
                        ...
                        if text_length:
                            ...
                            content_columns = [text_content[i : i + length] for i in range(0, text_length, length)]
            ...
            self._effective_column_widths[column] = max(lengths) + len(self.style.cell_row_content_format) - 2
    ...
    def _get_cell_width(self, row: Row, column: int) -> int:
        ...
        column_width = self._column_widths[column] if column in self._column_widths else 0
    ...
    def _init_styles(cls) -> None:
        ...
        borderless = (
            TableStyle().set_horizontal_border_chars("=").set_vertical_border_chars(" ").set_default_crossing_char(" ")
        )
