...
class CompletionsCommand(Command):
    ...
    arguments: ClassVar[list[Argument]] = [
        helpers.argument("shell", "The shell to generate the scripts for.", optional=True)
    ]
    options: ClassVar[list[Option]] = [
        helpers.option("alias", None, "Alias for the current command.", flag=False, multiple=True)
    ]
...
"""
    ...
    def handle(self) -> int:
        ...
        if shell not in self.SUPPORTED_SHELLS:
            raise ValueError(f"[shell] argument must be one of {', '.join(self.SUPPORTED_SHELLS)}")
    ...
    def render_bash(self) -> str:
        ...
        opts = [f"--{opt.name}" for opt in sorted(self.application.definition.options, key=lambda o: o.name)]
        ...
        for cmd in sorted(self.application.all().values(), key=lambda c: c.name or ""):
            ...
            options = " ".join(
                f"--{opt.name}".replace(":", "\\:") for opt in sorted(cmd.definition.options, key=lambda o: o.name)
            )
            cmds_opts += [
                f"            ({command_name})",
                f'            opts="${{opts}} {options}"',
        ...
        return TEMPLATES["bash"] % {
            ...
            "compdefs": "\n".join(f"complete -o default -F {function} {alias}" for alias in aliases),
        }
        ...
        for cmd in sorted(self.application.all().values(), key=lambda c: c.name or ""):
            ...
            if len(cmd_path) == 1:
                ...
                if namespace not in namespaces:
                    cmds.append(f"complete -c {script_name} -f -n " f"'__fish{function}_no_subcommand' -a {namespace}")
                # Now complete the command
                subcmds = [name.split(" ")[-1] for name in self.application.all(namespace)]
                cmds.append(
                    f"complete -c {script_name} -f -n '__fish_seen_subcommand_from "
                    f"{namespace}; and not __fish_seen_subcommand_from {' '.join(subcmds)}' "
                    f"-a {cmd_name} -d '{sanitize(cmd.description)}'"
                )
                condition = f"__fish_seen_subcommand_from {namespace}; " f"and __fish_seen_subcommand_from {cmd_name}"

            cmds_opts += [
                f"# {cmd.name}",
                *[
                    f"complete -c {script_name} " f"-n '{condition}' " f"-l {opt.name} -d '{sanitize(opt.description)}'"
                    for opt in sorted(cmd.definition.options, key=lambda o: o.name)
                ],
                "",  # newline
            ]
