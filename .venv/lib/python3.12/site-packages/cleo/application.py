...
class Application:
    ...
    def set_solution_provider_repository(self, solution_provider_repository: SolutionProviderRepository) -> None:
    ...
    def add(self, command: Command) -> Command | None:
        ...
        if not command.name:
            raise CleoLogicError(f'The command "{command.__class__.__name__}" cannot have an empty name')
    ...
    def render_error(self, error: Exception, io: IO) -> None:
        ...
        trace = ExceptionTrace(error, solution_provider_repository=self._solution_provider_repository)
    ...
    def _configure_io(self, io: IO) -> None:
        ...
        if io.input.has_parameter_option(["--no-interaction", "-n"], True) or (
            io.input._interactive is None and io.input.stream and not io.input.stream.isatty()
        ):
