...
class Formatter:
    ...
    def format_and_wrap(self, message: str, width: int) -> str:
        ...
        for match in self.TAG_REGEX.finditer(message):
            ...
            if not (seen_open or tag):
                ...
                formatted, current_line_length = self._apply_current_style(text, output, width, current_line_length)
                output += formatted
            elif seen_open:
        ...
        formatted, current_line_length = self._apply_current_style(message[offset:], output, width, current_line_length)
        output += formatted
    ...
    def _apply_current_style(self, text: str, current: str, width: int, current_line_length: int) -> tuple[str, int]:
        if not text:
