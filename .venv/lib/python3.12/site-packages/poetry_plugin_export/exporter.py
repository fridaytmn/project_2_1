...
class Exporter:
    ...
    def _export_generic_txt(self, cwd: Path, output: IO | str, with_extras: bool, allow_editable: bool) -> None:
        ...
        if indexes and self._with_urls:
            ...
            indexes_header = ""
            has_pypi_repository = any(r.name.lower() == "pypi" for r in self._poetry.pool.all_repositories)
            ...
            for repository in self._poetry.pool.all_repositories:
                if not isinstance(repository, HTTPRepository) or repository.url not in indexes:
                    continue

                url = repository.authenticated_url if self._with_credentials else repository.url
                parsed_url = urllib.parse.urlsplit(url)
                if parsed_url.scheme == "http":
                    indexes_header += f"--trusted-host {parsed_url.netloc}\n"
                if not has_pypi_repository and repository is self._poetry.pool.repositories[0]:
    ...
    _export_constraints_txt = partialmethod(_export_generic_txt, with_extras=False, allow_editable=False)

    _export_requirements_txt = partialmethod(_export_generic_txt, with_extras=True, allow_editable=True)
