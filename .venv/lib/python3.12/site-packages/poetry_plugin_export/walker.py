...
def get_python_version_region_markers(packages: list[Package]) -> list[BaseMarker]:
    ...
    for region in regions:
        ...
        if region.min is not None:
            ...
            marker_name = "python_full_version" if region.min.precision > 2 else "python_version"
        ...
        if region.max is not None:
            ...
            marker_name = "python_full_version" if region.max.precision > 2 else "python_version"
...
def get_project_dependency_packages(
    ...
    locked_extras = {
        canonicalize_name(extra): [canonicalize_name(dependency) for dependency in dependencies]
...
def walk_dependencies(
    ...
    while dependencies:
        ...
        locked_package = get_locked_package(requirement, packages_by_name, nested_dependencies)
        ...
        for require in locked_package.requires:
            if require.is_optional() and not any(
                require in locked_package.extras.get(feature, ()) for feature in locked_package.features
            ):
                continue
        ...
        if key not in nested_dependencies:
            ...
            nested_dependencies[key].marker = nested_dependencies[key].marker.union(requirement.marker)
...
def get_locked_package(
    ...
    for package in candidates:
        ...
        if old_decision is not None and not old_decision.marker.intersect(dependency.marker).is_empty():
    ...
    if overlapping_candidates:
        filtered_compatible_candidates = [
            package for package in compatible_candidates if package in overlapping_candidates
        ]
