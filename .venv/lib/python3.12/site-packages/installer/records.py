...
class InvalidRecordEntry(Exception):
    ...
    def __repr__(self):
        return "InvalidRecordEntry(elements={!r}, issues={!r})".format(self.elements, self.issues)
...
class RecordEntry:
    ...
    def __repr__(self) -> str:
        return "RecordEntry(path={!r}, hash_={!r}, size={!r})".format(self.path, self.hash_, self.size)

    def __eq__(self, other):
        if not isinstance(other, RecordEntry):
            return NotImplemented
        return self.path == other.path and self.hash_ == other.hash_ and self.size == other.size
...
def parse_record_file(rows: Iterable[str]) -> Iterator[Tuple[str, str, str]]:
    ...
    for row_index, elements in enumerate(reader):
        if len(elements) != 3:
            message = "Row Index {}: expected 3 elements, got {}".format(row_index, len(elements))
