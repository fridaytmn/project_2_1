...
class WheelDestination:
    ...
    def write_script(self, name: str, module: str, attr: str, section: "ScriptSection") -> RecordEntry:
...
class SchemeDictionaryDestination(WheelDestination):
    ...
    def write_file(
        ...
        if scheme == "scripts":
            with fix_shebang(stream, self.interpreter) as stream_with_different_shebang:
                return self.write_to_fs(scheme, path_, stream_with_different_shebang, is_executable)
    ...
    def write_script(self, name: str, module: str, attr: str, section: "ScriptSection") -> RecordEntry:
        ...
        with io.BytesIO(data) as stream:
            entry = self.write_to_fs(Scheme("scripts"), script_name, stream, is_executable=True)
    ...
    def _compile_bytecode(self, scheme: Scheme, record: RecordEntry) -> None:
        ...
        dir_path_to_embed = os.path.dirname(os.path.join(self.scheme_dict[scheme], record.path))  # Without destdir
        for level in self.bytecode_optimization_levels:
            compileall.compile_file(target_path, optimize=level, quiet=1, ddir=dir_path_to_embed)
