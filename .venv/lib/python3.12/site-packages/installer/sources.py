...
class WheelFile(WheelSource):
    ...
    def dist_info_dir(self) -> str:
        ...
        top_level_directories = {path.split("/", 1)[0] for path in self._zipfile.namelist()}
        dist_infos = [name for name in top_level_directories if name.endswith(".dist-info")]
    ...
    def validate_record(self, *, validate_contents: bool = True) -> None:
        ...
        try:
            ...
            record_mapping = {record[0]: record for record in parse_record_file(record_lines)}
        ...
        for item in self._zipfile.infolist():
            ...
            if record_args is None:
                issues.append(f"In {self._zipfile.filename}, {item.filename} is not mentioned in RECORD")
                continue
            ...
            if item.filename == f"{self.dist_info_dir}/RECORD":
                ...
                if record.hash_ is not None or record.size is not None:
                    ...
                    issues.append(f"In {self._zipfile.filename}, RECORD file incorrectly contains hash / size.")
                continue
            if record.hash_ is None or record.size is None:
                # Report empty hash / size
                issues.append(f"In {self._zipfile.filename}, hash / size of {item.filename} is not included in RECORD")
            if validate_contents:
                data = self._zipfile.read(item)
                if not record.validate(data):
                    issues.append(f"In {self._zipfile.filename}, hash / size of {item.filename} didn't match RECORD")
